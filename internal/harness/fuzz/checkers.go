...
// master-width-honored, master-stack-split, maximized-fold-intact) and the
// Tabbed-aware one (tabbed-strip-flat). The no-crash invariant is the
...
func CheckStep(hub *workspace.Hub, s *sim.SimSwayClient, wsNames []string, ev sway.Event, step int, res *Result) {
	...
	checkTabbedStripFlat(hub, s, wsNames, ev, step, res)
}
